...
bind_global :: proc(name: string) -> BindingId {
    for binding_index := 0; binding_index < Emitter.state.global_env.count; binding_index += 1 {
        if Emitter.state.global_env.names[binding_index] == name {
            return BindingId(binding_index)
        }
    }

    binding_id := BindingId(Emitter.state.global_env.count)
    Emitter.state.global_env.names[Emitter.state.global_env.count] = name
    Emitter.state.global_env.count += 1
...
bind_native_global :: proc(name: string, native_proc: NativeFunction) {
    ...
    Emitter.state.global_env.values[int(binding_id)] = Value(cast(^Object)native_function)
}
...
begin_proto :: proc(name: string, param_count: int) {
    ...
    Emitter.const_pool = make([dynamic]Value)
    Emitter.child_protos = make([dynamic]^Proto)
...
end_proto :: proc() {
    ...
    function_object^ = ProtoFunctionObject{
        header = Object{kind = .PROTO_FUNCTION},
        name   = Emitter.name,
        impl  = proto,
    }

    Emitter.state.entry_function = function_object
}
...
const_int :: proc(value: i64) -> int {
    ...
    append(&Emitter.const_pool, Value(value))
...
const_string :: proc(text: string) -> int {
    ...
    const_index := len(Emitter.const_pool)
    append(&Emitter.const_pool, Value(cast(^Object)string_object))
...
emit_get_global :: proc(dst: int, binding_id: BindingId) {
    record_slots(dst)
...
emit_set_global :: proc(src: int, binding_id: BindingId) {
    record_slots(src)
